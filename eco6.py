...
class EconomySimulator:
    ...
    def check_game_status(self):
        ...
        if self.rounds >= self.max_rounds:
            self.game_over = True
            self.message = "游戏结束！已达到最大回合数"
            return False, self.message
        
        if self.GDP >= self.target_gdp and abs(self.budget_deficit) <= self.max_deficit:
            self.game_over = True
            self.game_won = True
            self.message = (f"恭喜！你成功实现了经济增长目标 (GDP: ¥{self.GDP:.1f}亿)，"
                           "同时维持了财政健康！")
            return False, self.message
        
        if self.budget_deficit > self.max_deficit:
            self.game_over = True
            self.message = (f"财政危机！赤字过高 (¥{self.budget_deficit:.1f}亿)。游戏结束！")
            return False, self.message
    ...
    def draw(self, surface):
        ...
        if not self.game_over:
            ...
            # 绘制鼠标悬停提示
            if self.execute_button.hovered:
                pygame.draw.rect(surface, HIGHLIGHT, self.execute_button.rect, 3, border_radius=8)
                tip = small_font.render("应用当前税收和支出政策", True, HIGHLIGHT)
                surface.blit(tip, (self.execute_button.rect.x, self.execute_button.rect.y - 30))
    ...
    def draw_message_box(self, surface):
        ...
        # 绘制标题
        title = "游戏胜利！" if self.game_won else "游戏结束"
        title_surf = header_font.render(title, True, HIGHLIGHT)
        surface.blit(title_surf, (box_x + (box_width - title_surf.get_width()) // 2, box_y + 20))
        
        # 绘制消息文本
        message_surf = normal_font.render(self.message, True, TEXT_COLOR)
        surface.blit(message_surf, (box_x + (box_width - message_surf.get_width()) // 2, 
                                 box_y + (box_height - message_surf.get_height()) // 2))
        
        # 绘制按钮
        self.restart_button = Button(box_x + box_width//2 - 120, box_y + box_height - 60, 100, 40, "重新开始", self.reset_game)
        self.quit_button = Button(box_x + box_width//2 + 20, box_y + box_height - 60, 100, 40, "退出游戏", self.quit_game)
...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            # 处理游戏结束按钮
            if game.game_over and game.restart_button and game.quit_button:
                result = game.restart_button.handle_event(event)
                if result == "reset":
                    game = EconomySimulator()
                
                result = game.quit_button.handle_event(event)
                if result == "quit":
                    running = False
